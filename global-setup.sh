...
for i in $DEBIAN_BUSTER_BUILDS; do
    echo "deb http://deb.debian.org/debian buster main" > /build/debian-$i/etc/apt/sources.list
...
for i in $DEBIAN_STRETCH_BUILDS; do
    echo "deb http://deb.debian.org/debian stretch main" > /build/debian-$i/etc/apt/sources.list
    cp /prep/prepare-common.sh /build/debian-$i
    chroot /build/debian-$i /bin/bash prepare-common.sh
done

for i in $DEBIAN_JESSIE_BUILDS; do
    echo "deb http://archive.debian.org/debian jessie main" > /build/debian-$i/etc/apt/sources.list
    cp /prep/prepare-common.sh /build/debian-$i
    chroot /build/debian-$i /bin/bash prepare-common.sh
done
...
for i in $RASPBIAN_BUILDS; do
    export QEMU_CPU=arm1176
    echo "deb http://archive.raspbian.org/raspbian stretch main" > /build/raspbian-$i/etc/apt/sources.list
...
for i in $DEBIAN_SQUEEZE_BUILDS; do
    echo "deb http://archive.debian.org/debian squeeze main" > /build/debian-$i/etc/apt/sources.list
